...
class TestDataSegmentGenerator:
    ...
    def test_initialization_basic(self):
        ...
        segmenter = DataSegmentGenerator(
            mask=self.mask, data=self.data, dt=self.dt, t0=self.t0
        )
    ...
    def test_initialization_validation(self):
        ...
        with pytest.raises(ValueError, match="Mask and data must have the same length"):
            DataSegmentGenerator(mask=self.mask[:-10], data=self.data, dt=self.dt)

        # Invalid dt
        with pytest.raises(ValueError, match="Sampling interval dt must be positive"):
            DataSegmentGenerator(mask=self.mask, data=self.data, dt=-0.1)

    def test_segment_finding(self):
        """Test that segments are correctly identified."""
        segmenter = DataSegmentGenerator(mask=self.mask, data=self.data, dt=self.dt)
    ...
    def test_empty_segments(self):
        ...
        segmenter = DataSegmentGenerator(mask=all_gap_mask, data=self.data, dt=self.dt)
    ...
    def test_single_segment(self):
        ...
        segmenter = DataSegmentGenerator(mask=no_gap_mask, data=self.data, dt=self.dt)
    ...
    def test_mask_with_zeros(self):
        ...
        segmenter = DataSegmentGenerator(
            mask=mask_with_zeros, data=self.data, dt=self.dt
        )
    ...
    def test_tapered_mask(self):
        ...
        segmenter = DataSegmentGenerator(mask=tapered_mask, data=self.data, dt=self.dt)
    ...
    def test_different_t0(self):
        ...
        t0 = 5.0
        segmenter = DataSegmentGenerator(
            mask=self.mask, data=self.data, dt=self.dt, t0=t0
        )
    ...
    def test_from_gap_generator(self, _mock_gap_window_generator):
        ...
        mock_instance.generate_window.assert_called_once_with(
            apply_tapering=True, taper_definitions=None
        )
    ...
    def test_edge_tapering_left(self):
        ...
        segments = segmenter.get_time_segments(apply_window=True, left_edge_taper=10)
    ...
    def test_edge_tapering_right(self):
        ...
        segments = segmenter.get_time_segments(apply_window=True, right_edge_taper=10)
    ...
    def test_edge_tapering_both_edges(self):
        ...
        segments = segmenter.get_time_segments(
            apply_window=True, left_edge_taper=15, right_edge_taper=20
        )
    ...
    def test_edge_tapering_multiple_segments(self):
        ...
        segments = segmenter.get_time_segments(
            apply_window=True, left_edge_taper=10, right_edge_taper=15
        )
