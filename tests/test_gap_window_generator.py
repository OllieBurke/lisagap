...
class TestGapWindowGenerator:
    ...
    def gap_mask_generator(self):
        ...
        gap_definitions = {
            "planned": {"test_gap": {"rate_per_year": 1, "duration_hr": 2}},
            "unplanned": {"test_gap": {"rate_per_year": 0.5, "duration_hr": 1}},
        }
        return GapMaskGenerator(sim_t=sim_t, gap_definitions=gap_definitions)
    ...
    def test_gap_labels_extraction(self):
        ...
        gap_mask_gen = GapMaskGenerator(sim_t=sim_t, gap_definitions=gap_definitions)
        gap_gen = GapWindowGenerator(gap_mask_gen)

        assert set(gap_gen.planned_labels) == {"maintenance", "calibration"}
        assert set(gap_gen.unplanned_labels) == {
            "hardware_failure",
            "communication_loss",
        }
    ...
    def test_apply_smooth_taper_basic(self, gap_mask_generator):
        ...
        taper_defs = {"planned": {"test_gap": {"lobe_lengths_hr": 0.5}}}
    ...
    def test_validate_taper_definitions(self, gap_mask_generator):
        ...
        valid_taper_defs = {"planned": {"test_gap": {"lobe_lengths_hr": 0.5}}}
    ...
    def test_attribute_inheritance(self, gap_mask_generator):
        ...
        assert np.array_equal(
            gap_gen.unplanned_rates, gap_mask_generator.unplanned_rates
        )
    ...
    def test_proportional_tapering_static_method(self):
        ...
        tapered = GapWindowGenerator.apply_proportional_tapering(
            mask, dt=1.0, short_taper_fraction=0.25, medium_taper_fraction=0.05
        )
    ...
    def test_proportional_tapering_parameters(self):
        ...
        assert (
            n_intermediate2 > n_intermediate1
        ), "Higher taper fraction should create more intermediate values"
    ...
    def test_gap_merging_basic(self, gap_mask_generator):
        ...
        mask_no_merge, stats_no_merge = gap_gen.generate_window(merge_close_gaps=False)
