...
class GapWindowGenerator:
    ...
    def generate_window(
        ...
        mask = self.gap_mask_generator.generate_mask(
            include_planned=include_planned, include_unplanned=include_unplanned
        )
        ...
        if merge_close_gaps:
            mask, merge_stats = self._merge_close_gaps(mask, min_freq_resolution_hz)
    ...
    def _merge_close_gaps(
        ...
        original_valid = (
            np.sum(mask != 0) if not self.treat_as_nan else np.sum(~np.isnan(mask))
        )
        ...
        merged_valid = (
            np.sum(merged_mask != 0)
            if not self.treat_as_nan
            else np.sum(~np.isnan(merged_mask))
        )
    ...
    def _validate_taper_definitions(
        ...
        for kind in taper_gap_definitions:
            ...
            if not isinstance(taper_gap_definitions[kind], Mapping):
                raise TypeError(
                    f"taper_gap_definitions['{kind}'] must be a dictionary."
                )
    ...
    def apply_proportional_tapering(
        ...
        for i, (start, end) in enumerate(zip(gap_starts, gap_ends)):
            ...
            tapered_mask[win_start:win_end] = np.minimum(
                tapered_mask[win_start:win_end], taper_window
            )
